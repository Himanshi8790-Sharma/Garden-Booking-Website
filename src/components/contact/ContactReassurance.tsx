...
import { FiMessageSquare, FiPackage, FiCalendar, FiEye } from "react-icons/fi";
import { contactReassurancePoints } from "./contactData";

if (typeof window !== "undefined") {
  gsap.registerPlugin(ScrollTrigger);
}

const iconsList = [FiMessageSquare, FiPackage, FiCalendar, FiEye];
...
export default function ContactReassurance() {
  ...
  return (
    <section
      ref={sectionRef}
      aria-label="Why Contact Venue Concierge"
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        <div className="text-center max-w-2xl flex flex-col items-center">
          ...
          <p className="mt-4 text-[var(--foreground-muted)] font-sans font-light text-base md:text-lg">
            We provide direct, 1-on-1 venue consultations to ensure your event shift runs flawlessly.
          </p>
        </div>

        {/* 4 Points Grid */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
          {contactReassurancePoints.map((item, index) => {
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="mt-2 text-xs sm:text-sm font-sans font-light text-[var(--foreground-muted)] leading-relaxed">
                    {item.desc}
                  </p>
                </div>
